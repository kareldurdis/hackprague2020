...
export const sumIncomes = () => getIncomes().reduce((sum, transaction) => sum + transaction.amount, 0);

export function useStorage<T>(key: string, initial: T): [T, Dispatch<SetStateAction<T | undefined>>] {
    const [value, setValue] = useLocalStorage(key, initial);
    return [value || initial, setValue];
}

export function useDreams() {
    return useStorage('dreams', dreamsMock);
}
